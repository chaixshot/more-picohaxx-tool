...
function Select-InteractiveMenu([string]$header = "", [string[]]$options, [int]$defaultIndex = 0, [int]$maxVisible = 10) {
    ...
    # Store state in a reference object (hashtable) so nested functions can modify it
    ...
    function Render-Menu {
        ...
        for ($i = $startIndex; $i -le $endIndex; $i++) {
            ...
            if ($i -eq $selected) {
                ...
                [Console]::Write("${paddedLine}`n")
                [Console]::ResetColor()
            } else {
                [Console]::Write("${paddedLine}`n")
            }

            if (-not $state.IsLogged) {
        ...
        [Console]::Write("${paddedInfo}`n")
        [Console]::ResetColor()

        if (-not $state.IsLogged) {
            Write-Transcript $pageInfo
        }
