...
function BackupUserData([string]$backupPath) {
    ...
    if ($totalChunks -eq 1 -and $ranges[0].StartSector -eq $obPInfo.iStart) {
...
function Get-AllocatedRanges($obPInfo) {
    <#
    .SYNOPSIS
        Returns a list of allocated sector ranges for the userdata partition.
    .DESCRIPTION
        Reads the filesystem superblock, detects the filesystem type (F2FS or ext4),
        and delegates to the appropriate handler which scans allocation tables to
        produce a minimal list of [StartSector, Sectors] ranges covering all used data.
        Contiguous gaps smaller than 128 MiB (64 segments / 32768 blocks) are merged
        into the surrounding range to limit the number of EDL commands.
        Returns $null on any failure so callers fall back to full-partition backup.
    #>
    ...
    if (-not (Execute-EdlCommand (BuildCommand -obPInfo $obSB -isTemp $true) -silent $true)) {
        Write-Log "Failed to read superblock for range detection." "Warning"
        return $null
    }

    $sSBFile = BuildFileName -obPInfo $obSB -isTemp $true
    if (-not (Test-Path $sSBFile)) { return $null }

    try { $sbBytes = [System.IO.File]::ReadAllBytes($sSBFile) }
    catch { return $null }

    $sbBase = 1024
    if ($sbBytes.Length -lt ($sbBase + 0x80)) { return $null }

    $isF2FS = ($sbBytes[$sbBase + 0] -eq 0x10 -and $sbBytes[$sbBase + 1] -eq 0x20 -and
        $sbBytes[$sbBase + 2] -eq 0xF5 -and $sbBytes[$sbBase + 3] -eq 0xF2)
    if ($isF2FS) {
        return (Get-AllocatedRanges-F2FS -obPInfo $obPInfo -sbBytes $sbBytes -sbBase $sbBase)
    }

    $isExt4 = ($sbBytes[$sbBase + 0x38] -eq 0x53 -and $sbBytes[$sbBase + 0x39] -eq 0xEF)
    if ($isExt4) {
        return (Get-AllocatedRanges-Ext4 -obPInfo $obPInfo -sbBytes $sbBytes -sbBase $sbBase)
    }
...
function Get-AllocatedRanges-F2FS($obPInfo, $sbBytes, $sbBase) {
    <#
    .SYNOPSIS
        F2FS: scans the SIT table and active log pointers to produce a
        list of allocated sector ranges with 128 MiB gap-merging.
    #>

    if ($sbBytes.Length -lt ($sbBase + 0x60)) { return $null }
    ...
    if ($blockSize -eq 0 -or $blocksPerSeg -eq 0 -or $cpBlkAddr -eq 0) { return $null }
    ...
    if (-not (Execute-EdlCommand (BuildCommand -obPInfo $obCP -isTemp $true) -silent $true)) { return $null }
    $sCPFile = BuildFileName -obPInfo $obCP -isTemp $true
    if (-not (Test-Path $sCPFile)) { return $null }
    try { $cpBytes = [System.IO.File]::ReadAllBytes($sCPFile) } catch { return $null }
    if ($cpBytes.Length -lt 0x84) { return $null }
    ...
    if (-not (Execute-EdlCommand (BuildCommand -obPInfo $obSIT -isTemp $true) -silent $true)) { return $null }
    $sSITFile = BuildFileName -obPInfo $obSIT -isTemp $true
    if (-not (Test-Path $sSITFile)) { return $null }
    try { $sitBytes = [System.IO.File]::ReadAllBytes($sSITFile) } catch { return $null }
...
function Get-AllocatedRanges-Ext4($obPInfo, $sbBytes, $sbBase) {
    <#
    .SYNOPSIS
        ext4: scans the Block Group Descriptor table to produce a list of
        allocated sector ranges with 128 MiB gap-merging.
    #>

    if ($sbBytes.Length -lt ($sbBase + 0x200)) { return $null }
    ...
    if ($totalBlocks -eq 0 -or $blockSize -eq 0 -or $blocksPerGroup -eq 0) { return $null }
    ...
    if ($bgdSectors -gt 65536) {
        Write-Log "BGD table too large (${cYellow}$bgdSectors sectors${cReset}); cannot scan ranges." "Warning"
        return $null
    }
    ...
    if (-not (Execute-EdlCommand (BuildCommand -obPInfo $obBGD -isTemp $true) -silent $true)) { return $null }
    $sBGDFile = BuildFileName -obPInfo $obBGD -isTemp $true
    if (-not (Test-Path $sBGDFile)) { return $null }
    try { $bgdBytes = [System.IO.File]::ReadAllBytes($sBGDFile) } catch { return $null }
